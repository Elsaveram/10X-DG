...
new.clusters <- c("astrocytes", "astrocytes", "OPC", "astrocytes", "astrocytes", 
                  "mature OL", "RGL-2", "microglia", "immature neurons", "pericytes", 
                  "neuroblast", "astrocytes", "mature OL", "young OL", "immature granule", 
                  "activated OPC","mature granule", "NPC", "interneurons", "young OL", "early OPC", "RGL-1", "endothelial", "macrophages", "VLMC", "OPC")
...
TSNEPlot(pbmc, do.label=T, pt.size = 0.5, no.axes=TRUE, label.size=5)

save (pbmc, file = file.path(project_folder, 'Negative_Low_High_filtering_h_060818.Robj') )
                                                                                                                                                                                                                                                                                                                                                                                     ?TSNEPlot

#Change of names
pbmc <- SetAllIdent(pbmc, "res.1.4")
current.clusters <- 0:25
new.clusters <- c("a-astrocytes", "a-astrocytes", "k-OPC", "a-astrocytes", "a-astrocytes", 
                  "n-mature OL", "c-RG-like active", "z-microglia", "g-immature neurons", "z-pericytes", 
                  "e-neuroblast", "a-astrocytes", "n-mature OL", "m-young OL", "f-immature granule", 
                  "l-activated OPC","h-mature granule", "d-NPC", "i-interneurons", "m-young OL", "j-early OPC", "b-RG-like quiescent", "z-endothelial", "z-macrophages", "z-VLMC", "k-OPC")
for (i in current.clusters) {
  pbmc <- RenameIdent(object = pbmc, old.ident.name = i, 
                      new.ident.name = new.clusters[i + 1])
}
...
cluster.names <- plyr::mapvalues(pbmc@meta.data$res.1.4, from = current.clusters, to = new.clusters)
names(cluster.names) <- colnames(pbmc@data)
pbmc <- AddMetaData(pbmc, cluster.names, "Clusters_2")
TSNEPlot(pbmc, do.label=T, pt.size = 0.5, no.axes=TRUE, label.size=5)
...
# Count of cells by sample and cluster 
pbmc <- SetAllIdent(pbmc, "Clusters")
Transcriptpercluster<- with(pbmc@meta.data, table(sample.number.column, Clusters))
write.csv(Transcriptpercluster, file = file.path(project_folder, 'Negative_Low_High_filtering_h_TranscriptsClusters0618.csv') )
...
pbmc <- SetAllIdent(pbmc, "Clusters")
pbmc <- SubsetData(pbmc, ident.use = c("astrocytes", "RG-like 1", "RG-like 2"), subset.raw = T)

#microglia for Zilai

microglia <- SubsetData(pbmc, ident.use = "microglia", subset.raw = T)
save (microglia, file = file.path(project_folder, 'DG_microglia_EV_for_Zilai.Robj') )
...
genes_of_interest <- list(
  ...
  #'NSC lineage_4'              = c("ECEV4806transgene","Aldh1l1","Id4", "Gfap", "Sox9",  "Sox2", "Lpar1", "Nes", "Cdk1","Ccnd2","Neurod1", "Eomes", "Dcx", "Sox11", "Hsd11b1", "Dynlrb2", "Tmem212", "Fam183b", "Kcne1l", "Thrsp", "Hes6", "Igfbp5", "Ube2c", "Cks2", "Islr2"))
  #'NSC lineage_3'              = c("ECEV4806transgene","Aldh1l1","Id4", "Gfap", "Sox9",  "Sox2", "Lpar1", "Nes", "Cdk1","Ccnd2","Neurod1", "Eomes", "Dcx", "Sox11", "Hsd11b1", "Dynlrb2", "Kcne1l", "Hes6", "Ube2c", "Cks2", "Islr2" ))
  #"Genes for violin" = c("Id4", "Gfap", "Sox9", "Sox2", "Lpar1", "Nes"))
  #'NSC lineage 2'              = c("ECEV4806transgene", "Gfap", "Sox9", "Id4","Aldh1l1", "Sox2", "Lpar1", "Nes", "Cdk1","Ccnd2","Neurod1", "Eomes", "Dcx", "Sox11", "Hsd11b1", "Mia", "Dynlrb2", "Kcne1l", "Igfbp5", "Ube2c", "Cks2", "Islr2" ))
  #'Transcription factor RG-like' =c("Zbtb20", "Fos", "Ascl1", "Hes6","Sox4", "Egr1", "Foxj1"),
  #'Transcription factor NPC'     =c("Hmgb2", "Sox11"),
  #'Transcription factor Neuroblast' =c("Zbtb20", "Nfix", "Sox4", "Sox11", "Nfib", "Neurod1"))
  #'RNA FISH probe' =c("Hsd11b1", "Ascl1", "Cdk1", "Tfap2c", "Eomes", "Igfbpl1", "Dcx", "Calb2", "Aldoc", "Mia", "Dynlrb2","Hes6", "Foxj1", "Kcne1l", "Igfbp5", "Sox11", "Hmgb2", "Ube2c", "Cks2", "Islr2", "Neurod1"))
  #'OL lineage'                = c("Sox10", "Olig2", "Sox9", "Pdgfra", "Cspg4","Mki67", "Bmp4", "Mal", "Mog" ))
  ...
  #'Oligodendrocyte lineage'  = c( "Sox10", "Olig2", "Pcna", "Pdgfra", "Cspg4", "Slc1a3", "Mbp", "Mal", "Bmp4"))
 ...
 # 'RGL_h'                      = c("Lpar1", "Nes", "Prom1", "Hes5", "Tfap2c", "Rhcg", "Wnt8b"))
  ...
  #"Sheila's genes"              = c("Camk2a","Neurod1", 'Dlx1', "Cdkn1b","Cdkn1a", "Cdkn2a", "Nf1", "Trp53", "Pten")) 
  #'Cell Cycle genes_h'         = c("Cdk1","Aurkb", 'Top2a', 'Mki67'))  
  #'Genes for paper'            = c("Lpar1","Ascl1", 'Aldh1l1', 'Aqp4', "Sox9", "Id4", "Gfap")) 
#'Cell cycleGenes for paper' = c("Mcm2","Pcna", 'Mki67', 'Ccnd2', 'Nes', 'Lpar1', 'Prom1'))   
  #'NSC genes for paper'        = c("Sox9", "Gfap", "Id4", "Sox2", "Aldh1l1", "Aqp4", "Lpar1","Nes", "Ascl1")) 
  #'NSC genes for paper'        = c("Thrsp", "Hes6", "Igfbp5", "Dynlrb2", "Tmem212", "Fam183b"))
  'NSC genes for paper3'        = c("Id4", "Sox9", "Lpar1"))
...
for ( focus_genes in names(genes_of_interest) ) {
  ...
  tiff( file.path(analysis_folder,"Plots", paste(focus_genes, "Cluster.png")), res=100, height=1000, width=1600)
  FeaturePlot(object = pbmc, features.plot = genes, cols.use = c("grey", "blue"), reduction.use = "tsne")
  dev.off()
  
  tiff( file.path(analysis_folder,"Plots", paste(focus_genes, "Heatmap.png")), res=100, height=1000, width=2200)
  ...
  for ( identity in c( "sample.number.names", "Clusters")) {
    ...
    tiff( file.path(analysis_folder,"Plots", paste(focus_genes, identity, "Dot.tiff")), res=100, height=800, width=800)
    DotPlot(pbmc,genes.plot = genes, x.lab.rot = T, dot.scale = 8, plot.legend=T, col.min = 0)
    dev.off()
    
    # Generate the violin plot
    tiff( file.path(analysis_folder,"Plots", paste(focus_genes, identity, "Violn.tiff")), res=100, height=1000, width=vln_plot_width)
...
RGL <- c("RG-like quiescent", "RG-like active", "NPC", "activated OPC")
focus.clusters <- RGL
focus.cells <- WhichCells( pbmc, ident = focus.clusters, max.cells.per.ident = 200)

G1_S_genes <- CellCycle_Genes_Whitfield_2002_ %>% collect %>% .[["G1/S"]]
S_genes <- CellCycle_Genes_Whitfield_2002_ %>% collect %>% .[["S"]]
G2_M_genes <- CellCycle_Genes_Whitfield_2002_ %>% collect %>% .[["G2/M"]]
Mitosis_genes <- CellCycle_Genes_Whitfield_2002_ %>% collect %>% .[["M"]]
M_G1_genes <- CellCycle_Genes_Whitfield_2002_ %>% collect %>% .[["M/G1"]]
all_Cell_Cycle_genes<-as.vector(as.matrix(CellCycle_Genes_Whitfield_2002_[,c("G1/S", "S", "G2/M", "M", "M/G1")]))
my_cell_cycle_genes <-c("Lmnb1", "Ezh2", "Mcm6", "Pcna", "Mki67", "Ccnd2", "Rrm2", "Smc4", "Tyms", "Hmgb3")

genes_of_interest_CC <-my_cell_cycle_genes
#genes_of_interest_CC <- c("Hmgb2","Hmgb3")

top_cc_genes <- pbmc.markers %>% group_by(cluster) %>% filter(cluster %in% focus.clusters) %>% filter(gene %in% genes_of_interest_CC) %>% mutate(order = match(cluster, focus.clusters)) %>% top_n(30,  avg_logFC) %>% arrange(order)
DoHeatmap(object = pbmc, cells.use = focus.cells, genes.use = top_cc_genes$gene, slim.col.label = TRUE, group.order = focus.clusters, col.low="#330099",col.mid = "#000000", col.high = "#CC0000", disp.min = -1, disp.max = 1)

?DoHeatmap

# Write the top10 genes by cluster to a CSV file
write.csv( top30, file = file.path(project_folder, 'Negative_Low_High_filtering_h_top30_Custers.csv') )
...
RGL <- c("RG-like active","RG-like quiescent")
focus.clusters <- Astrocytes

new.clusters <- 
top_genes <- pbmc.markers %>% group_by(cluster) %>% filter(cluster %in% focus.clusters) %>% mutate(order = match(cluster, focus.clusters)) %>% top_n(15,  avg_logFC) %>% arrange(order)
...
DoHeatmap(object = pbmc, genes.use = top15$gene, slim.col.label = TRUE, remove.key = TRUE)
...
cluster_1 <- SubsetData(pbmc, ident.use = "RGL-1", subset.raw = T)
avg.cluster_1 <- log1p(AverageExpression(cluster_1))
avg.cluster_1$gene <- rownames(avg.cluster_1)

cluster_2 <- SubsetData(pbmc, ident.use = "RGL-2", subset.raw = T)
...
merged_clusters['x'] = merged_clusters["RGL-1"]
merged_clusters['y'] = merged_clusters["RGL-2"]


highlight.gene <- c("Ccnd2")
...
ggplot(merged_clusters, aes(x,y)) + geom_point()+ geom_text(aes(label=gene), vjust = -0.5) + xlab("RG-like quiescent") + ylab("RG-like active") + stat_smooth(method='lm', formula=y ~ x) 
...
cluster_2 <- SubsetData(pbmc, ident.use = "RGL-1", subset.raw = T)
avg.cluster_2 <- log1p(AverageExpression(cluster_2))
avg.cluster_2$gene <- rownames(avg.cluster_2)
...
merged_clusters['x'] = merged_clusters["astrocytes"]
merged_clusters['y'] = merged_clusters["RGL-1"]

highlight.gene <- "Fam183b"
mycolours <- c("highlight" = "red", "normal" = "grey50")

merged_clusters$highlight <- ifelse(merged_clusters$gene == highlight.gene, "highlight", "normal")
ggplot(merged_clusters, aes(x,y)) + geom_point() + geom_point(size = 1, aes(colour = highlight))
ggplot(merged_clusters, aes(x,y)) + geom_point()+ stat_smooth(method='lm', formula=y ~ x, se=F) +geom_text(aes(label=gene))
